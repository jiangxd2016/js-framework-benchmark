...
function isFrameworkDir(keyedDir: string, framework: string): boolean {
  ...
  const pnpmLockPath = path.resolve(framework,"pnpm-lock.yaml")

  return fs.existsSync(packageJSONPath) && (fs.existsSync(packageLockJSONPath)||fs.existsSync(pnpmLockPath));
}
